...
description: "Summarize external content such as YouTube videos, web pages, PDFs, podcasts, transcripts, or pasted text into a faithful Japanese HTML report under reports/. Use for requests containing 要約, まとめ, summarize, recap, digest, transcript, 文字起こし, YouTube要約, or レポート化. Fetch the actual source first, preserve every major topic, add useful tables or diagrams, include source metadata and the required disclaimer, update index.html, clean temporary files, and commit and push the result."
...
- 配色は共通の `assets/stash-theme.css` / `assets/stash-theme.js`（黒＝既定 / 淡い白）を `<head>` で同期読み込みする。`data-theme` と `localStorage["stash-theme"]` を index・全レポートで共有し、選んだ色が引き継がれる。右上の切替スイッチは JS が自動生成する（テンプレート側に `.theme-switch` は不要）。
- レポート自身の `:root` は `--sidebar-w`・`--content-max`・`--radius` などレイアウト変数のみでもよい（色は共通 CSS が `html[data-theme]` で上書きする）。既存 [index.html](../../../index.html) と同じカラートークンを使う。
...
<!-- 共通の配色テーマ（黒＝dark 既定 / 淡い白＝light）。index と各レポートで
     localStorage["stash-theme"] を共有し、右上に切替スイッチを自動生成する。 -->
<link rel="stylesheet" href="../assets/stash-theme.css" />
<script src="../assets/stash-theme.js"></script>
<title>{{タイトル}}</title>
...
<script>
  // 共通テーマ（data-theme）に合わせる。stash-theme.js が <head> で先に data-theme を付与済み。
  const themeAttr = document.documentElement.getAttribute('data-theme');
  const isDark = themeAttr ? themeAttr === 'dark'
